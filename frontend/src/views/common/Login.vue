...
import { getDomains, getWallet } from '../../api/billing'
...
const {
    ...
    showAddressCredential, userSettings, addressPassword, wallet, userJwt
} = useGlobalState()
...
const domainCreditCosts = ref({})

const refreshDomainCosts = async () => {
    // /user_api/billing/domains requires authenticated user context
    if (!userJwt.value) {
        domainCreditCosts.value = {}
        return
    }
    try {
        ...
        for (const row of rows || []) {
            if (row?.domain) {
                map[row.domain] = Number(row.credit_cost || 0)
            }
        }
        domainCreditCosts.value = map
    } catch {
        domainCreditCosts.value = {}
    }
}

const selectedDomainCreditCost = computed(() => {
    if (!emailDomain.value) return null
    const val = domainCreditCosts.value[emailDomain.value]
    return Number.isFinite(val) ? val : null
})

const canCreateAddressByCoin = computed(() => {
    if (selectedDomainCreditCost.value === null) return true
    return Number(wallet.value?.balance_credit || 0) >= Number(selectedDomainCreditCost.value)
})

const domainOptionsWithCost = computed(() => {
    return (domainsOptions.value || []).map((domain) => {
        const cost = domainCreditCosts.value[domain.value]
        if (!Number.isFinite(cost)) return domain
        return {
            ...domain,
            label: `${domain.label} (${cost} ${t('coinUnit')})`,
        }
    })
})
...
onMounted(async () => {
    ...
    if (userJwt.value) {
        try {
            ...
            // no-op: wallet panel can still render without this prefetch
        }
    }
    await refreshDomainCosts()
                    ...
                    <n-form>
                        ...
                        <n-input-group>
                            ...
                            <n-select v-model:value="emailDomain" :consistent-menu-width="false"
                                :options="domainOptionsWithCost" />
                        </n-input-group>
                        <p v-if="selectedDomainCreditCost !== null" style="margin: 8px 0 0; opacity: 0.8;">
                            {{ t('domainCostHint', { cost: selectedDomainCreditCost, coin: t('coinUnit') }) }}
                        </p>
                        <n-alert
                            v-if="selectedDomainCreditCost !== null && !canCreateAddressByCoin"
                            type="warning"
                            :show-icon="false"
                            :bordered="false"
                        >
                            {{ t('insufficientCoinForDomain', { need: selectedDomainCreditCost, have: wallet.balance_credit || 0 }) }}
                        </n-alert>
                        ...
                        <n-button type="primary" block secondary strong @click="newEmail" :loading="loading"
                            :disabled="!canCreateAddressByCoin">
                            <template #icon>
                                <n-icon :component="NewLabelOutlined" />
                            </template>
                            {{ t('getNewEmail') }}
